...
def load_roi_timeseries(path: str | Path) -> np.ndarray:
    """Load a ROI time series array.

    Expects a NumPy-compatible file (``.npy`` or ``.npz``) containing a 2D
    array of shape ``(T, n_rois)``.
    """
    path = Path(path)
    if path.suffix == ".npz":
        data = np.load(path)
        # pick the first array if keys are unknown
